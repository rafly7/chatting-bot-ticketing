...
module.exports = {
    ...
    async handle(
        ...
        BotsApp: BotsApp,
        args: string[],
        commandHandler: Map<string, Command>
    ): Promise<void> {
        try {
            ...
            if (!args[0]) {
                ...
                commandHandler.forEach((element) => {
                    helpMessage += format(
                        HELP.TEMPLATE,
                        prefixes[0] + element.name,
                        element.description
                    );
                });
                client
                    .sendMessage(BotsApp.chatId!, helpMessage, MessageType.text)
                    .catch((err) =>
            ...
            let command: Command | undefined = commandHandler.get(args[0]);
            if (command) {
                var triggers: string = " | ";
                ...
                if (command.demo?.isEnabled) {
                    ...
                    helpMessage +=
                        format(
                            HELP.COMMAND_INTERFACE_TEMPLATE,
                            triggers,
                    ...
                    if (command.demo.text instanceof Array) {
                        for (var i in command.demo.text) {
                            var button: proto.Message.ButtonsMessage.IButton = {
                                buttonId: "id" + i,
                                buttonText: {
                                    displayText: command.demo.text[i],
                                },
                                type: 1,
                            };
                            buttons.push(button);
                        ...
                        buttons.push({
                            buttonId: "id1",
                            buttonText: { displayText: command.demo.text },
                            type: 1,
                        });
                    }
                    const buttonMessage = {
