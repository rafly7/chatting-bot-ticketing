...
const clearance = async (
    ...
    if (isBlacklist) {
        if (BotsApp.isGroup) {
            ...
            if (
                !BotsApp.fromMe &&
                !BotsApp.isSenderSUDO &&
                !BotsApp.isSenderGroupAdmin
            ) {
                return false;
            }
        } else if (!BotsApp.fromMe && !BotsApp.isSenderSUDO) {
            console.log(
                chalk.blueBright.bold(`[INFO] Blacklisted Chat or User.`)
            );
            return false;
        }
    } else if (
        BotsApp.chatId === "917838204238-1634977991@g.us" ||
        BotsApp.chatId === "120363020858647962@g.us" ||
        BotsApp.chatId === "120363023294554225@g.us"
    ) {
        console.log(
    ...
    if (BotsApp.isCmd && !BotsApp.fromMe && !BotsApp.isSenderSUDO) {
        if (config.WORK_TYPE.toLowerCase() === "public") {
            if (BotsApp.isGroup) {
                await client.getGroupMetaData(BotsApp.chatId!, BotsApp);
                if (
                    adminCommands.indexOf(BotsApp.commandName!) >= 0 &&
                    !BotsApp.isSenderGroupAdmin
                ) {
                    console.log(
                    ...
                    sudoCommands.indexOf(BotsApp.commandName!) >= 0 &&
                    !BotsApp.isSenderSUDO
                ) {
                    console.log(
                    ...
                    let messageSent: boolean = await Users.getUser(
                        BotsApp.chatId
                    );
                    if (messageSent) {
                        console.log(
                            chalk.blueBright.bold(
                                "[INFO] Promo message had already been sent to " +
                                    BotsApp.chatId
                            )
                        );
                        return false;
            ...
            config.WORK_TYPE.toLowerCase() != "public" &&
            !BotsApp.isSenderSUDO
        ) {
            console.log(
                chalk.redBright.bold(`[INFO] commmand `),
                chalk.greenBright.bold(`${BotsApp.commandName}`),
                chalk.redBright.bold(`not executed in private Work Type.`)
            );
