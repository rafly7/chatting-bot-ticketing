...
const resolve = async function (
    ...
    try {
        jsonMessage = JSON.stringify(messageInstance);
    } catch (err) {
    ...
    BotsApp.mimeType = messageInstance.message
        ? Object.keys(messageInstance.message)[0] ===
          "senderKeyDistributionMessage"
            ? Object.keys(messageInstance.message)[2]
            : Object.keys(messageInstance.message)[0] === "messageContextInfo"
            ? Object.keys(messageInstance.message)[1]
            : Object.keys(messageInstance.message)[0]
        : null;
    BotsApp.type =
        BotsApp.mimeType === "imageMessage"
            ? "image"
            : BotsApp.mimeType === "videoMessage"
            ? "video"
            : BotsApp.mimeType === "conversation" ||
              BotsApp.mimeType == "extendedTextMessage"
            ? "text"
            : BotsApp.mimeType === "audioMessage"
            ? "audio"
            : BotsApp.mimeType === "stickerMessage"
            ? "sticker"
            : BotsApp.mimeType === "senderKeyDistributionMessage" &&
    ...
    BotsApp.isTextReply =
        ...
        messageInstance.message?.extendedTextMessage?.contextInfo?.stanzaId
            ? true
            : false;
    BotsApp.replyMessageId =
        messageInstance.message?.extendedTextMessage?.contextInfo?.stanzaId;
    BotsApp.replyParticipant =
        messageInstance.message?.extendedTextMessage?.contextInfo?.participant?.replace(
            /:.*@/g,
            "@"
        );
    BotsApp.replyMessage =
    ...
    BotsApp.body =
        BotsApp.mimeType === "conversation"
            ...
            : BotsApp.mimeType == "buttonsResponseMessage"
            ? messageInstance.message?.buttonsResponseMessage
                  ?.selectedDisplayText
            : null;
    BotsApp.isCmd = prefixRegex.test(BotsApp.body!);
    BotsApp.commandName = BotsApp.isCmd
        ? BotsApp.body!.slice(1)
              .trim()
              .split(/ +/)
              .shift()!
              .toLowerCase()
    ...
    BotsApp.isReplyImage = messageInstance.message?.extendedTextMessage
        ?.contextInfo?.quotedMessage?.imageMessage
        ? true
        : false;
    BotsApp.imageCaption = BotsApp.isImage
        ? messageInstance.message?.imageMessage?.caption
        : null;
    BotsApp.isGIF =
        BotsApp.type === "video" &&
        messageInstance.message?.videoMessage?.gifPlayback;
    BotsApp.isReplyGIF = messageInstance.message?.extendedTextMessage
        ?.contextInfo?.quotedMessage?.videoMessage?.gifPlayback
        ? true
        : false;
    BotsApp.isSticker = BotsApp.type === "sticker";
    BotsApp.isReplySticker = messageInstance.message?.extendedTextMessage
        ?.contextInfo?.quotedMessage?.stickerMessage
        ? true
        : false;
    BotsApp.isReplyAnimatedSticker =
        messageInstance.message?.extendedTextMessage?.contextInfo?.quotedMessage?.stickerMessage?.isAnimated;
    BotsApp.isVideo =
        BotsApp.type === "video" &&
        !messageInstance.message?.videoMessage?.gifPlayback;
    BotsApp.isReplyVideo = BotsApp.isTextReply
        ? jsonMessage.indexOf("videoMessage") !== -1 &&
          !messageInstance.message?.extendedTextMessage?.contextInfo
              ?.quotedMessage?.videoMessage?.gifPlayback
        : false;
    BotsApp.isAudio = BotsApp.type === "audio";
    BotsApp.isReplyAudio = messageInstance.message?.extendedTextMessage
    ...
    BotsApp.sender =
        BotsApp.isGroup && messageInstance.message && BotsApp.fromMe
            ? BotsApp.owner
            : BotsApp.isGroup && messageInstance.message
            ? messageInstance!.key!.participant!.replace(/:.*@/g, "@")
            : !BotsApp.isGroup
            ? BotsApp.chatId
            : null;
    BotsApp.isSenderSUDO = SUDOstring.includes(
        BotsApp.sender?.substring(0, BotsApp.sender!.indexOf("@"))!
    );
